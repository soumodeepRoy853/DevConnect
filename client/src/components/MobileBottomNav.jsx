...
import { Home, Compass, Users, Users2, Settings } from "lucide-react";
...
const MobileBottomNav = () => {
  ...
  const { auth } = useAuth();
  const navItems = auth?.user ? items : items.filter((item) => item.href !== "/settings");
        ...
        {navItems.map(({ href, label, Icon }) => {
          ...
          return (
            ...
            >
              ...
              <span className={isActive ? "border-b-2 border-primary-600" : ""}>{label}</span>
            </Link>
          );
        })}
      </div>
